...
def render_weekly_demand_staffing_chart(coverage_df, slot_labels):
    ...
    line_df = pd.DataFrame(
        {"Demand": weekly_df["demand"].to_numpy(), "Staffed": weekly_df["staffed"].to_numpy()},
        index=x_labels
    )
    ...
    total_under = float(np.maximum(weekly_df["demand"] - weekly_df["staffed"], 0).sum())
    total_over = float(np.maximum(weekly_df["staffed"] - weekly_df["demand"], 0).sum())
    c1.metric("Total under (avg/day sum)", f"{total_under:.1f}")
    c2.metric("Total over (avg/day sum)", f"{total_over:.1f}")
    c3.metric("Max |deviation| (avg/day)", f"{float(np.abs(deviation).max()):.1f}")
...
if st.button("Solve", type="primary"):
    ...
    # ---- Weekly single line chart (only one chart) ----
    ...
    st.download_button(
        ...
        file_name="naranjos_schedule.csv",
        mime="text/csv",
    )
    st.download_button(
        "Download coverage CSV",
        cov_df.to_csv(index=False).encode("utf-8"),
        file_name="naranjos_coverage.csv",
        mime="text/csv",
    )

    # Per-worker 7×13 tables with light-blue highlight for scheduled slots
    st.markdown("### Per-worker Schedule (7×13, color = scheduled)")
    workers = [str(w) for w in staff_df["name"].tolist()]

    def style_schedule(df):
        return df.style.apply(
            lambda s: ['background-color: #E6F4FF' if v == 1 else '' for v in s],
            axis=1
        )

    worker_tables = {}
    if not sched_df.empty:
        mat_template = np.zeros((7, 13), dtype=int)
        for w in workers:

...
# --------- Visualization helper (WEEKLY ONLY, AVERAGE PER DAY) ---------
def render_weekly_demand_staffing_chart(coverage_df, SLOT_LABELS):
    st.markdown("### Weekly Demand vs Staffing (average per day)")

    # Average across the 7 days so the y-axis aligns with the per-day cap (e.g., 2.5)
    ...
    line_df = pd.DataFrame({
        "Demand (avg/day)": weekly_df["demand"].to_numpy(),
        "Staffed (avg/day)": weekly_df["staffed"].to_numpy(),
    }, index=x_labels)
    st.line_chart(line_df, use_container_width=True)

    # Metrics: check the *daily* max absolute deviation from raw rows to match the cap semantics

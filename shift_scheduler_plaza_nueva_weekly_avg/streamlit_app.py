...
SLOT_LABELS = ["12-13","13-14","14-15","15-16","16-17","17-18",
...
with st.sidebar:
    ...
    max_dev = st.number_input("Max deviation per slot (daily)", min_value=0.0, value=2.5, step=0.1)
    ...
    agg_mode = st.selectbox("Weekly chart aggregation", ["Average per day", "Sum over week"], index=0,
                            help="Average keeps the y-axis around 0–3 and aligns with the 2.5 per-slot daily cap.")
...
if demand_file is not None:
    ...
    demand_df = pd.DataFrame([default_demand[d] for d in range(1,8)])
...
# --------- Weekly chart (Average or Sum) + constraint check ---------
def render_weekly_chart_and_checks(coverage_df, slot_labels, agg_mode):
    st.markdown("### Weekly Demand vs Staffing")

    if coverage_df is None or coverage_df.empty:
        st.warning("No coverage data to plot.")
        return

    # Per-row daily deviation
    ...
    line_df = pd.DataFrame({
        "Demand": weekly_df["demand"].astype(float).values,
        "Staffed": weekly_df["staffed"].astype(float).values
    }, index=x_labels)
    st.caption(subtitle)
    ...
    total_under = float(np.maximum(coverage_df["demand"]-coverage_df["staffed"],0).sum())
    total_over = float(np.maximum(coverage_df["staffed"]-coverage_df["demand"],0).sum())
...
if st.button("Solve", type="primary"):
    ...
    # Weekly chart + checks
    render_weekly_chart_and_checks(cov_df, SLOT_LABELS, agg_mode)

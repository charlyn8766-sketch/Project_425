...
st.set_page_config(page_title="Shift Scheduler — Weekly Chart Only (13 slots)", layout="wide")
st.title("Shift Scheduler — Weekly Chart Only (13 slots)")
st.caption("This build renders only the weekly aggregate line chart (no daily tables or worker grids).")

DAYS = list(range(1,8))
SLOTS = list(range(1,14))  # 13 slots
...
with st.sidebar:
    ...
    st.caption("Rows: Mon..Sun; Cols: 13 hourly slots (12..24). If omitted, a demo matrix is used.")
    demand_file = st.file_uploader("Upload demand.csv", type=["csv"], key="demand")

# Demo demand 7×13 if none provided
DEMO = np.array([
...
if demand_df.shape != (7,13):
    st.error(f"Demand CSV must be 7 rows × 13 columns. Got {demand_df.shape}.")
    st.stop()
...
if st.button("Solve", type="primary"):
    ...
    line_df = pd.DataFrame({"Demand": weekly["demand"], "Staffed": weekly["staffed"]},
                           index=[
                               ...
                               "18-19","19-20","20-21","21-22","22-23","23-00","00-01"
                           ])
